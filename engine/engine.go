...
type ServerOption func(*Engine)

// ServerBefore functions are executed on the HTTP request object before the
// request is decoded.
func ServerBefore(before ...RequestFunc) ServerOption {
	return func(s *Engine) { s.before = append(s.before, before...) }
}

// ServerAfter functions are executed on the HTTP response writer after the
// endpoint is invoked, but before anything is written to the client.
func ServerAfter(after ...ServerResponseFunc) ServerOption {
	return func(s *Engine) { s.after = append(s.after, after...) }
}
...
func ServerErrorEncoder(ee ErrorEncoder) ServerOption {
	return func(s *Engine) { s.errorEncoder = ee }
}
...
func ServerFinalizer(f ...ServerFinalizerFunc) ServerOption {
	return func(s *Engine) { s.finalizer = append(s.finalizer, f...) }
}
